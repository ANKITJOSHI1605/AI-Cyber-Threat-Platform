...
import { analyzeEmail, analyzeNetwork, analyzeUrl, API_URL, clearToken, createIncident, downloadIncidentReport, getAnalytics, getAuditLogs, getIncidents, getMe, getRecentScans, getSummary, getThreatIntelligence, getToken, getUsers, login, register, setIncidentStatus, setUserRole } from './api';
...
export default function App() {
  ...
  useEffect(() => { if (active === 'account' && user?.role === 'admin') Promise.all([getUsers(), getAuditLogs()]).then(([accounts, logs]) => { setUsers(accounts); setAuditLogs(logs); }).catch(requestError => setError(requestError.message)); }, [active, user]);
  ...
  async function changeRole(id, role) { try { const updated = await setUserRole(id, role); setUsers(current => current.map(item => item.id === id ? updated : item)); setAuditLogs(await getAuditLogs()); } catch (requestError) { setError(requestError.message); } }
  async function enrichUrl() { try { setThreatIntel(await getThreatIntelligence(result.normalized_url)); } catch (requestError) { setError(requestError.message); } }
  ...
  return <div className="app-shell">
    ...
    <main id="top"><header className="topbar"><div><p className="eyebrow">THREAT INTELLIGENCE CONSOLE</p><h1>{modules.find(item => item[0] === active)[2]}</h1></div><a className="docs-link" href={`${API_URL}/docs`} target="_blank" rel="noreferrer">API docs ↗</a></header>
      ...
      {active === 'url' && <><section className="scanner panel"><div className="panel-heading"><div><p className="eyebrow">LIVE ANALYSIS</p><h2>Inspect a suspicious URL</h2></div><span className="engine-badge">● Engine ready</span></div><form onSubmit={e => { setThreatIntel(null); run(e, analyzeUrl, url); }}><label htmlFor="url">URL or domain</label><div className="scan-row"><input id="url" value={url} onChange={e => setUrl(e.target.value)} placeholder="https://example.com/login" required /><button disabled={loading}>{loading ? 'Analyzing…' : 'Analyze URL →'}</button></div><div className="examples"><span>Try:</span>{examples.map(item => <button type="button" key={item} onClick={() => setUrl(item)}>{item}</button>)}</div></form></section><Signals result={result} />{result && <button className="intel-button" onClick={enrichUrl}>Check VirusTotal reputation</button>}{threatIntel && <section className="panel intel-card"><p className="eyebrow">EXTERNAL THREAT INTELLIGENCE</p><h2>{threatIntel.provider}: {threatIntel.status}</h2>{threatIntel.message ? <p>{threatIntel.message}</p> : <p>Malicious: {threatIntel.malicious} · Suspicious: {threatIntel.suspicious} · Harmless: {threatIntel.harmless} · Reputation: {threatIntel.reputation}</p>}</section>}{result && result.risk_score >= 30 && <button className="escalate" onClick={escalate}>Create security incident</button>}<section className="panel history"><div className="panel-heading"><h2>Recent URL scans</h2><span>{history.length} records</span></div>{history.map(item => <button key={item.id} onClick={() => setResult(item)}><span className={`verdict-dot ${item.verdict}`} /><span>{item.normalized_url}</span><b>{item.risk_score}</b><em>{item.verdict.replace('_', ' ')}</em></button>)}</section></>}
